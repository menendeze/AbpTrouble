...
using ecomercialapp.Migrations.SeedData;
...
namespace ecomercialapp.Tests
{
    public abstract class ecomercialappTestBase : AbpIntegratedTestBase<ecomercialappTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected ecomercialappTestBase()
        {
        ...
        protected void UsingDbContext(Action<ecomercialappDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ecomercialappDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ecomercialappDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ecomercialappDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ecomercialappDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ecomercialappDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ecomercialappDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ecomercialappDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ecomercialappDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ecomercialappDbContext, Task<T>> func)
        {
            T result;
